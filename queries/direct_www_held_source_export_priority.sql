with channel as (
    ...
    select
        ...
        path_group,
        sessions,
        dense_rank() over (partition by action_bucket order by sessions desc) as bucket_rank,
        round(100.0 * sessions / sum(sessions) over (partition by action_bucket), 2) as pct_within_bucket,
        round(100.0 * sessions / sum(sessions) over (), 2) as pct_of_all_held,
        case
            when dense_rank() over (order by sessions desc) <= 5 then 'export_now'
            else 'export_after_p1'
        end as export_priority
    from classified
    where action_bucket in ('hold_content_tool_direct', 'hold_tool_intent_direct')
      and default_channel_group = 'Direct'
)
select
    export_priority,
...
from held
order by
    case export_priority when 'export_now' then 1 else 2 end,
    sessions desc;
