...
| P0 | Raw import trust | `evidence_ready` | seo_data | 复核 28 个 partial raw imports，优先确认 GA4 page-location 与 hostname audit 缺行影响 | partial evidence CSV 已进入线上分支并完成 triage |
...
`scripts/ingest_selected_raw_sources.py` 已重跑。当前 `75` 个 selected raw source 中有 `28` 个被标记为 `partial`，明细见 `docs/phase3_raw_import_partial_evidence.csv`。

优先复核：

- `ga4/ga4_page_location_pii_token_check_2025-11-20_2026-05-19.csv`：缺 `42` 行，但导入率仍为 `99.96%`。
- `5-22/ga4_hostname_audit_28d_2026-04-24_to_2026-05-21.csv`：缺 `9` 行，导入率 `91.0%`，会影响 hostname audit 精度。
- `03 news/gsc/news_coverage_examples.csv`：`41` 行全被跳过，不能作为 GSC coverage 证据使用。
